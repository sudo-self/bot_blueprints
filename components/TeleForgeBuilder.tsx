...
export default function TeleForgeBuilder() {
  ...
  const getBotCodeString = () => {
    const apiToken = config.token === '*******' ? 'YOUR_BOT_TOKEN' : (config.token || 'YOUR_BOT_TOKEN');
    const adminId = config.adminId || '0';

    let code = `#!/usr/bin/env python3
# Generated by TeleForge Bot Builder
...
def handle_docs(message):
    ...
    try:
        ...
        bot.reply_to(message, f"✅ File '{message.document.file_name}' saved successfully!")
...
`;
  ...
  const handleExport = async () => {
    ...
    const requirements = "pyTelegramBotAPI" + (config.connectionType === 'webhook' ? "\nflask" : "");
    
    const readme = `# ${config.botName} - Telegram Bot

This bot was generated using TeleForge.
      ...
      <nav className="w-16 flex flex-col items-center py-6 border-r border-zinc-800 bg-zinc-900 z-10">
        <div className="mb-10">
          <div className="w-10 h-10 flex items-center justify-center overflow-hidden">
            <Image src="/icon.svg" alt="Project Icon" width={40} height={40} className="w-full h-full object-contain" />
          </div>
        </div>
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center overflow-hidden shrink-0">
                {config.botPicUrl ? (
                  ...
                  <Image src="/icon.svg" alt="Bot" width={40} height={40} className="w-full h-full object-cover" />
                )}
              </div>
              <div>
